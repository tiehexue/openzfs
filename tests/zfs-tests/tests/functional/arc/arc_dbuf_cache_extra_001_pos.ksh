...
# 3. Sample dbufstats.cache_extra_bytes: it must climb above its baseline.
...
# "cache_extra_bytes" kstat field, and requires fio.
#

verify_runnable "global"

is_linux || log_unsupported "Direct I/O setup is Linux specific"
command -v fio >/dev/null 2>&1 || log_unsupported "fio is required"
...
log_assert "dbuf_cache_extra grows under Direct I/O churn and decays after"
log_onexit cleanup
...
# arc_c_max: set arc_min == arc_max == c_max/8 (which clamps arc_c down to
...
(( pump < (32 * 1024 * 1024) )) && pump=$((32 * 1024 * 1024))
...
log_note "baseline cache_extra_bytes=$base_extra"
...
# The allowance must actually have grown, i.e. the mechanism engaged.
(( peak > base_extra )) || log_fail "cache_extra_bytes did not rise " \
    "under Direct I/O (baseline=$base_extra peak=$peak)"
...
log_pass "dbuf_cache_extra grew under Direct I/O and decayed afterwards"
